...
const TABLE_HEAD = [
  ...
  { id: 'sub-categories', label: 'subcategory', alignRight: false },
...
export default function UserPage() {
  ...
  useEffect(() => {
    ...
    const fetchproductData = async () => {
      try {
        const response = await axios.get('http://localhost:8006/products/get-products'); // Replace with your API endpoint
  ...
  const onDeleteHandler = (productid) => {
    axios
      .delete(`http://localhost:8006/products/delete-products/${productid}`)
      .then(() => {
              ...
              <Table>
                ...
                <TableBody>
                  {filteredsubcategory.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { _id, Name, categories, 'sub-categories': subCategories, thumbnail, createdAt } = row;
                      ...
                      <TableRow hover key={_id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                        ...
                        <TableCell component="th" scope="row" padding="none">
                          <Stack direction="row" alignItems="center" spacing={2}>
                            <img
                              alt="io"
                              src={`http://localhost:8006/uploads/products/${thumbnail}`}
                              width={100}
                              height={80}
