...
export default function UserPage() {
  ...
  useEffect(() => {
    ...
    const fetchsubcategoryData = async () => {
      try {
        const response = await axios.get('http://localhost:8006/subcategory/get-sub-category'); // Replace with your API endpoint
        if (response.status === 200) {
          setsubcategoryData(response.data.data); // Update the user data state with the fetched data
    ...
    // Call the fetchsubcategoryData function when the component mounts
  ...
  const onDeleteHandler = (categoryid) => {
    axios
      .delete(`http://localhost:8006/subcategory/delete-sub-category/${categoryid}`)
      .then(() => {
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - subcategoryData.length) : 0;
  const filteredsubcategory = applySortFilter(subcategoryData, getComparator(order, orderBy), filterName);
            ...
            <TableContainer sx={{ minWidth: 800 }}>
              <Table>
                <UserListHead
                  ...
                  rowCount={subcategoryData.length} // Update the row count based on fetched data
                ...
                <TableBody>
                  {filteredsubcategory.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    const { _id, Name, categories ,Image, createdAt } = row;
                      ...
                      <TableRow hover key={_id} tabIndex={-1} role="checkbox" selected={selectedUser}>
                        ...
                        <TableCell component="th" scope="row" padding="none">
                          <Stack direction="row" alignItems="center" spacing={2}>
                            <img
                              alt="io"
                              src={`http://localhost:8006/uploads/sub-category/${Image}`}
                              width={100}
                              height={70}
